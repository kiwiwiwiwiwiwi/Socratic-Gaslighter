...
# List of fallacies with brief, punchy explanations for the tooltip
FALLACIES = {
...
# Create a clean string layout for the dropdown question-mark tooltip
...
# -------------------------------------------------------------------
# 4. MORE DESCRIPTIVE HOME SCREEN
# -------------------------------------------------------------------
...
if st.session_state.game_over:
    if st.session_state.game_result == "WIN":
        st.balloons()
        st.success("🎉 VICTORY! You completely demolished the Gaslighter's credibility. They spark out, unable to compute your flawless logic!")
    else:
        st.error("💀 DEFEAT... You got completely gaslit. Your credibility hit 0%, and you walked away questioning your own sanity.")
...
with st.form(key="battle_action_form", clear_on_submit=True):
    ...
    with col_btn1:
        # Replaced label and mapped the cheat sheet to the built-in 'help' parameter text string
    ...
    with col_btn2:
        ...
        # BUG FIXED HERE: Changed from st.form_submit_input to st.form_submit_button
        submit_action = st.form_submit_button("💥 Execute Move", use_container_width=True)
